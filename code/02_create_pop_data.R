...
keep_zips   <- config::get()$keep_working_data
...
## 2010-2016 data (get the 2017 Vintage)
if (!file.exists(sprintf("%s/pcen_v2017_y1017.txt.zip", data_folder))) {
    download.file(
        url = paste0(
            "ftp://ftp.cdc.gov/pub/Health_Statistics/NCHS/",
            "Datasets/NVSS/bridgepop/2017/",
            "pcen_v2017_y1017.txt.zip"
        ),
        destfile = sprintf("%s/pcen_v2017_y1017.txt.zip",
                           data_folder)
    )
}
...
pop2010s <-
    read_fwf(sprintf("%s/pcen_v2017_y1017.txt.zip", data_folder),
             fwf_widths(
                 c(4, 2, 3, 2, 1, 1, 8, 8, 8, 8, 8, 8, 8, 8, 8),
                 c(
                     "series",
                     "fipst",
                     "fipsct",
...
collapse_df <- function(df) {
    newdf <- df %>%
        select(fipsihme, age, race, hispanic, starts_with("pop_est_")) %>%
        group_by(fipsihme, age, race, hispanic) %>%
        summarize_all(sum)
    return(newdf)
}
...
all_pops_long <- all_pops_wide %>%
    ...
    gather(year, pop_est, pop_est_1999:pop_est_2017) %>%
    mutate(year = as.integer(gsub("pop_est_", "", year)))

## Save ----
saveRDS(all_pops_long,
        sprintf("%s/pop_est_collapsed_long.RDS", data_folder))
